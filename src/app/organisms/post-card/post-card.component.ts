...
@Component({
    ...
    <article class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      @if (post().imageUrl) {
        <img class="max-h-[420px] w-full object-cover" [src]="post().imageUrl" alt="Imagen del post" loading="lazy" />
      }
      ...
      <div class="flex flex-wrap items-center gap-2 border-y border-slate-100 px-3 py-2">
        ...
        <ion-button fill="clear" size="small" (click)="save.emit(post().id)">
          <ion-icon [name]="post().saved ? 'bookmark' : 'bookmark-outline'" slot="start" aria-hidden="true" />
          Guardar
        </ion-button>
        ...
        @for (comment of post().comments; track comment.id) {
          <div class="rounded-lg bg-slate-50 px-3 py-2">
            <div class="flex items-center justify-between gap-2">
              ...
              <time class="text-[11px] text-slate-400" [dateTime]="comment.createdAt">{{ comment.createdAt | date: 'short' }}</time>
            </div>
            <p class="mt-1 text-sm leading-5 text-slate-700">{{ comment.content }}</p>
          </div>
        }
        <app-comment-form [postId]="post().id" (createComment)="comment.emit({ postId: post().id, content: $event })" />
      </div>
    </article>
  `
})
